# -*- coding: utf-8 -*-
"""按 cumcm-paper-writer 规范填充 2025B 论文问题一二三正文。"""
...
SRC = next(p for p in DOC_DIR.glob("*.docx") if not p.name.startswith("~$") and "问题" not in p.name)
OUT = DOC_DIR / "2025B论文_问题一二三.docx"
...
def add_para(
    ...
    *,
    ...
    """level: title / h1 / h2 / h3 / body / caption / formula"""
    p = doc.add_paragraph()
...
def add_table(doc, headers: list[str], rows: list[list[str]], caption: str):
...
def build() -> Document:
    ...
    # 复用已有背景段文字
    bg = old.paragraphs[3].text.strip()
    ir = old.paragraphs[6].text.strip()
    approach = old.paragraphs[7].text.strip()
    propose = old.paragraphs[10].text.strip()
    q1_req = old.paragraphs[11].text.strip()
    q2_req = old.paragraphs[12].text.strip()
    q3_req = old.paragraphs[13].text.strip()
    a1 = old.paragraphs[19].text.strip()
    a2 = old.paragraphs[21].text.strip()
    a3 = old.paragraphs[23].text.strip()
    ...
    add_para(doc, bg)
    add_para(doc, "图1  多尺寸SiC晶圆", level="caption")
    add_para(doc, ir)
    add_para(doc, approach)

    add_para(doc, "1.2 问题提出", level="h2")
    add_para(doc, propose)
    add_para(doc, q1_req)
    add_para(doc, q2_req)
    add_para(doc, q3_req)
    add_para(doc, "图2  问题关系图", level="caption")

    # —— 二、问题分析 ——
    add_para(doc, "二、问题分析", level="h1")
    add_para(
        doc,
        "本题研究对象为红外反射干涉光谱中的外延层厚度反演，本质是“光学相位—厚度”映射问题，"
        "并在碳化硅与硅两类材料上检验模型选择与可靠性。",
    )
    ...
    add_para(doc, "正文主要符号见表1。未列入的中间变量在首次出现处给出定义。", first_indent=True)
    add_table(
        doc,
        ["符号", "含义", "单位"],
        [
    ...
    add_para(
        doc,
        "设空气入射角为 θ0，外延层折射率为 n，折射角为 θ1。由 Snell 定律",
    )
    add_para(doc, "sin θ0 = n sin θ1，  cos θ1 = √(1 − sin²θ0 / n²)。", level="formula")
    add_para(
        doc,
        "其中 cos θ1 取物理可行的正分支。膜内一次往返光程差为 ΔL = 2 n d cos θ1。"
        "以波数 ν̃=1/λ 表示时，两束反射光的相位差写为",
        first_indent=False,
    )
    add_para(doc, "δ(ν̃) = 4π d ν̃ n cos θ1 + φr。", level="formula")
    add_para(
        doc,
        "其中 φr 为两界面反射引入的近似固定相位。它改变峰谷对应的干涉级次归属，"
    ...
    add_para(
        doc,
        "定义光学相位坐标 g(ν̃,θ0)=ν̃ √(n²−sin²θ0)。若两个同类极值跨越 Δm 个干涉级次，则",
        first_indent=False,
    )
    add_para(doc, "d = Δm / {2 [g(ν̃b,θ0) − g(ν̃a,θ0)]}。", level="formula")
    add_para(
        doc,
        "当窄波段内折射率近似为常数、且取相邻同类极值（Δm=1）时，得到问题一的核心解析关系：",
        first_indent=False,
    )
    add_para(doc, "d = 1 / [2 Δν̃ √(n² − sin²θ0)]。", level="formula")
    add_para(
        doc,
        "其中 d 以 µm、Δν̃ 以 cm⁻¹ 计时，右侧再乘 10⁴。"
        "当 θ0→0 时，公式退化为 d=1/(2nΔν̃)，可作为推导正确性的退化检验。",
        first_indent=False,
    )
    ...
    add_para(
        ...
        "由 d ∝ (n²−sin²θ0)^(−1/2) 可知，近法向时 Δd/d≈−Δn/n，例如 n 偏差 1% 将引起约 1% 的厚度反向偏差。"
        "同一晶圆应满足两角度共享厚度，故定义相对差",
    )
    add_para(doc, "ε = |d10 − d15| / [(d10+d15)/2] × 100%。", level="formula")
    add_para(
        doc,
        "并以重采样标准差的逆方差加权得到联合厚度，作为问题二最终报告值。",
        first_indent=False,
    )
    ...
    add_para(
        doc,
        "外延层内第 j 次往返相对前一束多乘复因子 q=r10 r12 exp(−2αd/cosθ1) exp(iδ)。"
    ...
    add_para(
        doc,
        "对 s、p 偏振，单层多次反射总振幅可写为",
    )
    add_para(
        doc,
        "r_tot = (r01 + r12 e^{2iβ}) / (1 + r01 r12 e^{2iβ})，  β = 2π d ν̃ n cos θ1。",
        level="formula",
    )
    add_para(
        doc,
        "非偏振反射率取二者强度平均。对称无吸收情形化为 Airy 形式，精细度由有效界面反射率 Ri 控制。"
        "数值实现中，非线性参数为 (d, Ri, 相位偏置)，线性增益与慢变基线用最小二乘剖面消元；"
        "可选用差分进化全局搜索再局部精修。",
        first_indent=False,
    )
    ...
    add_para(
        ...
        "谐波比 A2/A1≥0.08、有效反射率 Ri≥0.12、相对双光束的 RMSE 改善≥2%，以及 ΔAICc≥10。"
    ...
    add_para(
        ...
        "若 observable_multibeam=True，则采用多光束厚度；否则回退双光束厚度。"
    ...
    add_table(
        doc,
        ["数据集", "A2/A1", "有效反射率", "RMSE改善/%", "ΔAICc", "判定"],
        [
    ...
    add_para(
        doc,
        "硅片两角度多光束厚度相对差约 0.38%，逆方差加权联合结果为",
        first_indent=False,
    )
...
def write_markdown():
    text = """# 碳化硅外延层厚度的红外干涉测量及多光束干涉影响分析

> 本文件与 `2025B论文_问题一二三.docx` 同步，覆盖问题一二三正文主线（重述、分析、假设、符号、建模求解与汇总）。

## 一、问题重述
（见 Word；沿用并整理原稿背景与三问要求）

## 二、问题分析
### 2.1–2.3
问题一给斜入射双光束解析锚点；问题二完成 SiC 双角度稳健测厚；问题三做多光束可观测判定并完成 Si 测厚。

## 三、模型假设
1. 局部平行、光斑内等厚；2. n0=1；3. 透明波段反演；4. n_SiC=2.55、n_Si=3.42；5. 两角度共享厚度；6. 多光束需四项证据同时通过。

## 四、符号说明
见 Word 表1。

## 五、问题一
斜入射相位 δ=4π d ν̃ n cosθ1+φr；常折射率下
d=1/[2Δν̃√(n²−sin²θ0)]（µm/cm⁻¹ 时乘 10⁴）。

## 六、问题二
SiC：10° → 7.883 µm，15° → 7.791 µm；相对差≈1.16%；加权 d_SiC≈7.83 µm。

## 七、问题三
Si：谐波比≈0.20–0.22，判定可观测多光束；d_Si≈3.59 µm。
SiC：谐波比≈0.02–0.04，证据不足，不作多光束修正。
...
def main():
    ...
    try:
        ...
        alt = DOC_DIR / "2025B论文_问题一二三_填写稿.docx"
        doc.save(str(alt))
        saved = alt
    # 尝试覆盖原文件
    try:
