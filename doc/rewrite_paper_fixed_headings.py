...
OUT_ALT = DOC_DIR / "2025B论文_问题一二三_详细版_公式修订.docx"
...
def build():
    ...
    add(
        ...
        "再计算双角度相对差并做逆方差加权融合。全过程使用同一阈值与随机种子规则，保证两角度可比。",
    )
    ...
    add(
        doc,
        "问题二得到碳化硅外延层厚度约 7.83 µm，双角度相对差约 1.16%，并给出条件置信区间与折射率系统误差说明。"
        "该结果作为问题三多光束判定的对照基线。",
    )
    ...
    formula(doc, "q = r₁₀ r₁₂ exp(−2αd/cosθ₁) exp(iδ)", "14")
    follow(
        doc,
    ...
    formula(doc, "r_tot = (r₀₁ + r₁₂ e^{2iβ}) / (1 + r₀₁ r₁₂ e^{2iβ})， β=2π d ν̃ n cosθ₁", "15")
    follow(
        doc,
        "其中 β 与问题一相位结构同源。对称无吸收情形下，由上式得到 Airy 形式",
    )
    formula(doc, "T=1/[1+F sin²(δ/2)]， F=4Rᵢ/(1−Rᵢ)²， R_Airy=1−T", "16")
    follow(
        doc,
    ...
    formula(doc, "A₂/A₁≥0.08， Rᵢ≥0.12， RMSE改善≥2%， ΔAICc≥10", "17")
    follow(
        doc,
    ...
    slot(doc, "8", "多光束证据矩阵", "output/multibeam_evidence.png")
    slot(doc, "9", "模型质量对比", "output/model_quality.png")
    slot(doc, "10", "谐波比原始图", "output/raw_evidence/multibeam/harmonic_ratio_raw.png")
    slot(doc, "11", "有效反射率原始图", "output/raw_evidence/multibeam/effective_reflectivity_raw.png")
    slot(doc, "12", "Si 10°谐波频谱", "output/raw_evidence/multibeam/si_10_harmonic_spectrum_raw.png")
    slot(doc, "13", "SiC 10°谐波频谱", "output/raw_evidence/multibeam/sic_10_harmonic_spectrum_raw.png")
    slot(doc, "14", "附件3拟合证据图", "output/si_10_fit.png")
    slot(doc, "15", "附件4拟合证据图", "output/si_15_fit.png")
    ...
    formula(doc, "d_Si ≈ 3.59 µm", "18")
    follow(
        doc,
        "若硅片仍用双光束，联合厚度约 3.41 µm，相对多光束联合结果偏低约 5%。"
        "碳化硅因不满足四项证据门控，仍采用问题二的双光束结果。",
    )
    ...
    add(
        ...
        "结论：附件三、四存在可观察多光束干涉，d_Si≈3.59 µm；附件一、二证据不足，SiC 不作多光束修正，d_SiC≈7.83 µm。",
    )
