...
## v9 分析型证据可视化（2026-08-28）

### 完成内容

- 新增 `evidence_plotting.py`，生成13张带分析标识的论文证据图，与纯原始图目录分离。
- 波段资格图标注透明区、SiC二声子排除区、峰谷、被排除极值及合格数量。
- 级次回归图标注共享厚度、统计区间、峰谷差、双角度差和采用状态。
- 残差图标注稳健内点带、最大残差、稳健尺度和剔除比例。
- 门控图统一读取 `shared_thickness.V8_THRESHOLDS`，逐项显示实际值、阈值与通过/失败。
- 情景图同时展示本征/低/中/高厚度、条件统计误差棒、常折射率基线与系统范围。
- 最终决策图并排呈现SiC采用v8、Si回退Airy的证据链。
- 载流子审计图显示条件数、相关性、留段失败和“仅审计、不作为主厚度”。
- `main.py` 已接入自动输出到 `output/analysis_evidence/`。
- 详细版论文5.4/6.4新增分析图插图位，README与`model.md`新增图像说明。
